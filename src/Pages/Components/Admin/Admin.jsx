...
const Admin = () => {
  ...
  const Approval = async (ID, type, approve) => {
    try {
      ...
      await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/admin/${adminID}/approve/${type}/${ID}`, {
        method: 'POST',
        mode: "cors",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#042439] to-[#0a3c5c] text-white">
      ...
      <nav className="flex justify-between items-center p-4 md:px-10 bg-[#031d30] shadow-md">
        <NavLink to="/" className="flex items-center gap-3">
          ...
          <h1 className="text-xl md:text-2xl font-bold text-[#4E84C1]">Shiksharthee</h1>
        </NavLink>
        <div className="flex items-center gap-6">
          ...
          <button onClick={() => navigator('/')} className="px-4 py-2 bg-blue-600 rounded hover:bg-blue-700 transition">
            Logout
          </button>
        </div>
      </nav>
      ...
      <div className="flex flex-col md:flex-row justify-end gap-6 p-6 relative">
        <button onClick={() => setOpen(prev => !prev)} className="bg-green-600 hover:bg-green-700 text-white px-1 py-2 rounded shadow">
          Messages
        </button>
        <button 
  ...
  Course Requests
        ...
        <div className="bg-gray-700 rounded-lg shadow-lg mx-6 md:mx-10 p-6">
          ...
          <div className="space-y-4">
            {allmsg.map((msg, index) => (
              <div key={index} className="bg-gray-600 p-4 rounded hover:bg-gray-500 transition">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-8 p-6 md:p-10">
        ...
        <div className="bg-[#0b2f47] p-6 rounded-lg shadow hover:shadow-lg transition">
          <h2 className="text-center text-xl font-semibold mb-6 border-b border-white pb-2">Student Requests</h2>
          {StudentData.length > 0 ? StudentData.filter(s => s.Isapproved === "pending").map(student => (
            <div key={student._id} onClick={() => docDetails("student", student._id)}
              ...
              <p>Status: <span className="italic">{student.Isapproved}</span></p>
            </div>
        ...
        {/* Teacher Requests */}
        <div className="bg-[#0b2f47] p-6 rounded-lg shadow hover:shadow-lg transition">
          <h2 className="text-center text-xl font-semibold mb-6 border-b border-white pb-2">Teacher Requests</h2>
          {TeacherData.length > 0 ? TeacherData.filter(t => t.Isapproved === "pending").map(teacher => (
            <div key={teacher._id} onClick={() => docDetails("teacher", teacher._id)}
              className="bg-blue-700 hover:bg-blue-600 rounded-lg p-4 mb-4 cursor-pointer transition">
              <h3 className="text-lg font-bold">{teacher.Firstname} {teacher.Lastname}</h3>
              <p>Status: <span className="italic">{teacher.Isapproved}</span></p>
            </div>
        ...
        {/* Rejected Requests */}
        <div className="bg-[#0b2f47] p-6 rounded-lg shadow hover:shadow-lg transition">
          <h2 className="text-center text-xl font-semibold mb-6 border-b border-white pb-2">Rejected Requests</h2>
          {[...TeacherData, ...StudentData].filter(u => u.Isapproved === "rejected").map(user => (
            <div key={user._id} onClick={() => docDetails(user.role || "teacher", user._id)}
              ...
              <p>Remark: <span className="italic">{user.Remarks}</span></p>
            </div>
          ))}
        </div>
